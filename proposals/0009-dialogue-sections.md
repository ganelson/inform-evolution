...
If `DialogueKit` is present (but not otherwise), a new action, `talking to it about`
is provided in the Standard Rules which intercepts both **ASK X ABOUT C** and
**TELL X ABOUT C**, though only if C is a concept. The implementation is roughly like so:
